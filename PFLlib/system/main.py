...
def run(args):
    ...
    for i in range(args.prev, args.times):
        ...
        if model_str == "mlr":  # convex
            ...
            if "MNIST" in args.dataset:
                args.model = DNN(
                    1 * 28 * 28, 100, num_classes=args.num_classes
                ).to(args.device)
            elif "Cifar10" in args.dataset:
                args.model = DNN(
                    3 * 32 * 32, 100, num_classes=args.num_classes
                ).to(args.device)
            else:
                args.model = DNN(60, 20, num_classes=args.num_classes).to(
                    args.device
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ld", "--learning_rate_decay", type=bool, default=False
    )
    parser.add_argument(
        "-ldg", "--learning_rate_decay_gamma", type=float, default=0.99
    )
    ...
    parser.add_argument(
        "-t", "--times", type=int, default=1, help="Running times"
    )
    parser.add_argument(
        "-eg",
        "--eval_gap",
    ...
    parser.add_argument(
        "-glr", "--generator_learning_rate", type=float, default=0.005
    )
    parser.add_argument("-hd", "--hidden_dim", type=int, default=512)
    parser.add_argument("-se", "--server_epochs", type=int, default=1000)
    parser.add_argument(
        "-lf", "--localize_feature_extractor", type=bool, default=False
    )
    # SCAFFOLD / FedGH
    parser.add_argument(
        "-slr", "--server_learning_rate", type=float, default=1.0
    )
    # FedALA
    ...
    # FedKD
    parser.add_argument(
        "-mlr", "--mentee_learning_rate", type=float, default=0.005
    )
    ...
    if args.learning_rate_decay:
        print(
            "Local learing rate decay gamma: {}".format(
    ...
    print(
        "Fine tuning epoches on new clients: {}".format(
